...
func _initialize(data: Dictionary) -> void:
	if label_message == null:
		label_message = self.get_node("MarginContainer/VBoxContainer/LabelMessage") 
		print("ready loading _initialize")
	...
	if data.has("tips"):
		for t in data.tips:
			_tips.append(t)
		if _tips.size() > 0:
			print("ready loading _initialize label_message = %d",label_message)
