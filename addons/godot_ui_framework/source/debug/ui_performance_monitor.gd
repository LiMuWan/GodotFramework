...
func update_metrics() -> void:
	...
	_metrics.draw_calls.push_back(performance.get_monitor(Performance.RENDER_TOTAL_DRAW_CALLS_IN_FRAME))
	_metrics.node_count.push_back(performance.get_monitor(Performance.OBJECT_NODE_COUNT))
	_metrics.ui_updates.push_back(performance.get_monitor(Performance.PIPELINE_COMPILATIONS_CANVAS ))
